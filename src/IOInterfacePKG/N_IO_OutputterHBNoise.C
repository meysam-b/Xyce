...
{
  std::pair<OutputParameterMap::const_iterator, bool> result = output_manager.findOutputParameter(OutputType::HBNOISE);
  if (result.second)
  {
    for (std::vector<PrintParameters>::const_iterator it = (*result.first).second.begin(), end2 = (*result.first).second.end(); it != end2; ++it) 
    {
      PrintParameters hbnoise_print_parameters = (*it);

      output_manager.fixupPrintParameters(comm, hbnoise_print_parameters);

      Outputter::Interface *outputter;
      if (hbnoise_print_parameters.format_ == Format::STD) 
      {
        outputter = new Outputter::HBNoisePrn(comm, output_manager, hbnoise_print_parameters);
      }
      ...
      {
        Report::UserWarning0()
          << "HBNOISE output cannot be written in PROBE, RAW or Touchstone format, using standard format instead";
        hbnoise_print_parameters.format_ = Format::STD;
        outputter = new Outputter::HBNoisePrn(comm, output_manager, hbnoise_print_parameters); 
      }
      else
      {
        Report::UserWarning0() << "HBNOISE output cannot be written in " << hbnoise_print_parameters.format_ << " format, using standard format";
        hbnoise_print_parameters.format_ = Format::STD;
        outputter = new Outputter::HBNoisePrn(comm, output_manager, hbnoise_print_parameters);
      }

      output_manager.addOutputter(PrintType::HBNOISE, outputter);
    }
  }
}

//-----------------------------------------------------------------------------
// Function      : printHBNoiseHeader
